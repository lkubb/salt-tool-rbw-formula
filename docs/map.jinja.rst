...
We will see a quick setup to configure the ``tool_rbw`` formula for different DNS domain names and several roles.
...
Create per DNS configuration for ``tool_rbw`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per DNS domain name configuration files for the ``tool_rbw`` formulas under ``/srv/salt/tool_rbw/parameters/``.
...
.. code-block:: console
    ...
    mkdir -p /srv/salt/tool_rbw/parameters/dns:domain/
    ln -s dns:domain /srv/salt/tool_rbw/parameters/domain

We create a configuration for the DNS domain ``example.net`` in ``/srv/salt/tool_rbw/parameters/dns:domain/example.net.yaml``:
...
We create another configuration for the DNS domain ``example.com`` in the Jinja YAML template ``/srv/salt/tool_rbw/parameters/dns:domain/example.com.yaml.jinja``:
...
Create per role configuration for ``tool_rbw`` formula
~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~

Now, we can provides the per role configuration files for the ``tool_rbw`` formulas under ``/srv/salt/tool_rbw/parameters/``.

We create the directory for roles:

.. code-block:: console

    mkdir -p /srv/salt/tool_rbw/parameters/roles
...
We create a configuration for the role ``tool_rbw/server`` in ``/srv/salt/tool_rbw/parameters/roles/tool_rbw/server.yaml``:
...
We create another configuration for the role ``tool_rbw/client`` in ``/srv/salt/tool_rbw/parameters/roles/tool_rbw/client.yaml``:
...
.. code-block:: console
    ...
    salt 'server-*' grains.append roles tool_rbw/server

For the clients:

.. code-block:: console

    salt 'client-*' grains.append roles tool_rbw/client
...
.. code-block:: console
    ...
    mv /srv/salt/tool_rbw/parameters/dns:domain/  '/srv/salt/tool_rbw/parameters/dns!domain/'
...
.. code-block:: sls
    ...
    test-does-nothing-but-display-rbw-as-json:
      test.nop:
        - name: {{ rbw | json }}
...
.. code-block:: sls
    ...
    rbw-config-file-file-managed:
      file.managed:
        - name: {{ rbw.config }}
        - source: {{ files_switch(['example.tmpl'],
                                  lookup='rbw-config-file-file-managed'
                     )
                  }}
        - mode: 644
        - user: root
...
This ``sls`` file expose a ``rbw`` context variable to the jinja template which could be used like this:
...
.. code-block:: jinja
    ...
    winner of the merge: {{ rbw['winner'] }}
